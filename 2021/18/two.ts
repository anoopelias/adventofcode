...
async function solve() {
  const input = await fs.readFile("../../../aoc-files/2021/18/input");
  ...
  for (let i = 0; i < splits.length; i++) {
    for (let j = i + 1; j < splits.length; j++) {
      const m1 = magnitude(add(stringToTree(splits[i]), stringToTree(splits[j])));
      const m2 = magnitude(add(stringToTree(splits[j]), stringToTree(splits[i])));
      largest = Math.max(largest, m1, m2);
    }
  }
