...
async function solve() {
    const input = await fs.readFile("src/2021/24/input_3_4_5_6");
    const splits = input.toString().split("\n");
    splits.pop();

    for (let d11 = 9; d11 >= 1; d11--) {
        for (let d12 = 9; d12 >= 1; d12--) {
            for (let d13 = 9; d13 >= 1; d13--) {
                for (let d14 = 9; d14 >= 1; d14--) {
                    for (let z = 0; z <= 1000; z++) {
                        processAlu(splits, [d11, d12, d13, d14], z);
                    }
                }
            }
        }
