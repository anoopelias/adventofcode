...
function solve() {
    //const startState = new State([["A", "B"], ["D", "C"], ["C", "B"], ["A", "D"]]);
    const startState = new State([["D", "D"], ["C", "A"], ["B", "C"], ["B", "A"]]);
    const endState = new State([["A", "A"], ["B", "B"], ["C", "C"], ["D", "D"]]);
...
class State {
    ...
    print() {
        ...
        console.log("###" + this.rooms.map(room => room.length === 2 ? room[1] : ".").join("#") + "###");
        console.log("  #" + this.rooms.map(room => room.length >= 1 ? room[0] : ".").join("#") + "#");
        console.log("  #########");
        console.log("");
    }
    ...
    roomOutSteps(roomNo: number): number {
        return this.rooms[roomNo].length === 1 ? 2 : 1;
    }

    roomInSteps(roomNo: number): number {
        return this.rooms[roomNo].length === 0 ? 2 : 1;
    }
    ...
    canGoToRoom(letter: string) {
        ...
        return (this.rooms[num].length === 0 ||
            (this.rooms[num].length === 1 && this.rooms[num][0] === letter));
    }
